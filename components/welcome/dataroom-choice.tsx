...
export default function DataroomChoice({ dataroomId }: { dataroomId: string }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={STAGGER_CHILD_VARIANTS}
        className="grid w-full max-w-3xl grid-cols-1 divide-y divide-border rounded-md border border-border text-foreground md:grid-cols-2 md:divide-x md:divide-y-0"
      >
        <button
          onClick={() =>
            router.push({
        ...
        >
          <Sparkles className="pointer-events-none h-auto w-12 text-orange-500 sm:w-12" />
