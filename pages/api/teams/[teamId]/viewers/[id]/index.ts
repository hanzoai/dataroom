...
async function fetchAndCacheDurations(
  ...
  const cachedDurations = await kv.get(cacheKey);
  ...
  if (cachedDurations) {
    ...
    await kv.set(cacheKey, JSON.stringify(durationsMap), { ex: 600 });
  }
...
export default async function handle(
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      if (withDuration !== "true") {
        await kv.set(cacheKey, JSON.stringify(formattedViews), { ex: 600 }); // 10 min cache
      }
