...
export async function sendLinkCreatedWebhook({
  ...
  try {
    ...
    const webhooks = await subscribers(teamId, "link.created");
    ...
    const linkData = {
      ...
      url: link.domainId
        ...
        : `https://dataroom.hanzo.ai/view/${link.id}`,
      domain:
        link.domainId && link.domainSlug ? link.domainSlug : "dataroom.hanzo.ai",
